...
function resumeMemory() {
    Questions.findAllQuestions().then((docs) => {
        /**
        ...
        for (let i = 0; i < docs.length; i += 1) {
            ...
            for (
                let k = 0;
                k < dataset.length && dataset.length !== 0;
                k += 1
            ) {
            ...
            if (sessionFlag) {
                for (
                    let j = 0;
                    j < clusterQueue[relaventSessionId].length && !clusterflag;
                    j += 1
                ) {
                    if (
                        clusterNumber === clusterQueue[relaventSessionId][j] &&
                        clusterNumber !== 0
                    ) {
                        clusterflag = true;
                ...
                if (!clusterflag) {
                    ...
                    if (
                        clusterNumber >
                        similarityClusterCounter[relaventSessionId]
                    ) {
                        similarityClusterCounter[
                            relaventSessionId
                        ] = clusterNumber;
                    }
    ...
    return new Promise(function (resolve) {
        ...
        toxicityLoad
            ...
            .catch(function (exception) {
                ...
                console.error(
                    `Tf-Toxicity classifier fail: ${exception.message}`
                );
            });
    });
}
...
function tfToxicityQuestion(questionDoc, sessionId) {
    if (questionDoc) {
        checkTfToxicity(questionDoc.question).then((tfToxicityResult) => {
    ...
    return new Promise(function (resolve) {
        ...
        useLoad
            ...
            .catch(function (exception) {
                // eslint-disable-next-line no-console
                console.error(
                    `Tf-Universal sentence encoder fail: ${exception.message}`
                );
            });
    });
}
...
async function tfUseQuestion(questionDoc, sessionId) {
    ...
    if (sessionFlag) {
        for (
            let i = 0;
            i < dataset[relaventSessionId].length && insertedFlag === false;
            i += 1
        ) {
             ...
             * @type {number}
             */
            let productResult = math.dot(
                dataset[relaventSessionId][i][0].value,
                questionCode
            );
            if (productResult > similarityThreshold) {
                ...
                for (
                    let j = 0;
                    j < dataset[relaventSessionId][i].length;
                    j += 1
                ) {
                    // for each sentence in the same cluster, renew the weight, then set self weight
                    productResult = math.dot(
                        dataset[relaventSessionId][i][j].value,
                        questionCode
                    );
                ...
                if (dataset[relaventSessionId][i].length === 2) {
                    ...
                    ioInterface
                        ...
                        .emit(
                            'update-cluster-number',
                            dataset[relaventSessionId][i][0]._id,
                            clusterNumber
                        );
                    ...
                    dataset[relaventSessionId][
                        i
                    ][0].similarityCluster = clusterNumber;
                    dataset[relaventSessionId][
                        i
                    ][1].similarityCluster = clusterNumber;
                    // when a cluster a two question, suppose 2rd is center
                    dataset[relaventSessionId][i] = [
                        dataset[relaventSessionId][i][1],
                        dataset[relaventSessionId][i][0],
                    ];
                } else {
                    // when a cluster has three or more questions, unlock center question -> sort -> lock new center question
                    clusterNumber =
                        dataset[relaventSessionId][i][0].similarityCluster;
                    // change memory dataset
                    dataset[relaventSessionId][i][
                        dataset[relaventSessionId][i].length - 1
                    ].similarityCluster = clusterNumber;
                    // change database and socket-io dataset
                    Questions.updateClusterNumber({
                        questionId: questionDoc._id,
                    ...
                    dataset[relaventSessionId][i].sort(function (
                        first,
                        second
                    ) {
